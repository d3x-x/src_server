...
class Theft extends Branch {
	...
	startWork(player: Player) {
		if (this.workers.amount > 3) {
			hud.showNotification(player, 'error', 'Наведень поки немає, пiдлiтай пiзнiше!', true);
		...
		mp.blips.create(
			...
			{ model: 0, color: 48, name: 'Чекпоiнт' },
			player
		);
	}
	...
	finishWork(player: Player) {
		...
		if (worker.vehicle) {
			...
			mp.blips.delete(player, 'Чекпоiнт');
		}
	...
	private onUnlockVehicle(player: Player, vehicle: VehicleMp) {
		...
		hud.showNotification(
			player,
			'info',
			'Чудово! Достав тачку у вказане мiсце на картi.'
		);
	}
}
