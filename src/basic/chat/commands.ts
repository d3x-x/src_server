...
class ChatCommands {
	...
	getTemplate(text: string, command: number) {
		switch (command) {
			case COMMANDS.SAY:
				return `!{${colors.white}}[id] каже: ${text}`;
			case COMMANDS.ME:
				return `!{${colors.lilac}}[id] ${text}`;
			case COMMANDS.DO:
				return `!{${colors.lilac}}${text} ([id])`;
			case COMMANDS.TRY:
				return `!{${colors.lilac}}[id] ${text} (${random(0, 1) ? 'Вдало' : 'Невдало'})`;
			case COMMANDS.NRP:
				return `!{${colors.white}}[id]: (( ${text} ))`;
			case COMMANDS.SCREAM:
				return `!{${colors.white}}[id] крикнув: ${text}`;
			case COMMANDS.WHISPER: {
				const [, ...msg] = text.split(' ');
				return `!{${colors.white}}[id] сказав пошепки: ${msg.join(' ')}`;
			}
			case COMMANDS.TODO: {
				const [msg, action] = text.split('*');
				return `${msg}, !{${colors.lilac}} - сказав(ла) [id], ${action}`;
			}
			...
			case COMMANDS.FACTION:
				return `!{${colors.blue}}[Фракцiя] [rank] [name]: ${text}`;
			case COMMANDS.FACTION_NRP:
				return `!{${colors.blue}}[Фракцiя] [rank] [name]: (( ${text} ))`;
			case COMMANDS.ORG:
				return `!{${colors.blue}}[Органiзацiя] [rank] [name]: ${text}`;
