...
class VehicleLock {
	...
	toggle(player: Player, vehicle: VehicleMp) {
		...
		if (!owning.isOwner(vehicle, player)) {
			return hud.showNotification(player, 'error', 'У вас немає ключiв');
		}
		...
		hud.showNotification(player, 'info', locked ? 'Ви вiдкрили ТС' : 'Ви закрили ТС');
	}
	...
	private async open(player: Player, vehicle: VehicleMp) {
		...
		hud.showNotification(player, 'info', 'Ви вiдкрили ТС');
	}

	private triggerAlarm(player: Player, vehicle: VehicleMp) {
		policeCalls.createCall('Спроба злому ТС', vehicle.position, vehicle);
	}
}
