...
class VehicleTrade {
	...
	private showMenu(player: Player) {
		...
		if (!vehicles.length || !target) {
			return hud.showNotification(player, 'error', 'У вас поки немає транспорту', true);
		}
	...
	private offerToBuy(player: Player, vehicle: string, data: Data) {
		if (data.owners >= 3) {
			return hud.showNotification(
				player,
				'error',
				'Досягнуто лiмiт власникiв даного ТЗ',
				true
			);
		}
	...
	private async confirm(
		...
		if (!seller.vehicles.includes(vehicleId) || !player.isEnoughVehicleSlots()) {
			return mp.events.reject('Недостатньо мiсць для нових транспортних засобiв');
		}
