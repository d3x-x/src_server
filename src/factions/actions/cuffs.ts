...
class CuffsActions {
	...
	private useHandcuffs(player: Player) {
		...
		if (!handcuffs) {
			return hud.showNotification(player, 'error', 'У вас немає кайданкiв');
		}
	...
	private useCableTie(player: Player) {
		...
		if (!tie) {
			return hud.showNotification(player, 'error', 'У вас немає стяжок');
		}
	...
	private untiePlayer(player: Player) {
		...
		if (knife?.name !== 'knife') {
			return hud.showNotification(player, 'error', 'Вiзьмiть нiж у руки');
		}
