...
class ZoneCapture {
	...
	async startWar(player: Player) {
		...
		chat.sendToFaction(
			owner,
			`!{${chat.colors.orange}}УВАГА! Ваша територiя пiд загрозою`
		);
	}
	...
	private onPlayerDeath(player: Player, reason: string, killer: PlayerMp) {
		...
		const message = `!{${chat.colors.orange}}${killerName} !{${chat.colors.white}}вбив гравця !{${chat.colors.blue}}${targetName}`;
		mp.players.forEachInDimension(this.dimension, (entity: PlayerMp) => {
	...
	private checkCanStart(player: Player, defender: Faction) {
		...
		if (this.current || !gang || !gang.isLeader(player, true)) {
			return mp.events.reject('помилка доступу');
		}
		if (gang.name === defender.name) {
			return mp.events.reject('Ви не можете атакувати свою територiю');
		}
		if (defender.getPlayers(true).length < 3) {
			return mp.events.reject('Недостатньо учасникiв у бандi захисту');
		}
	}
}
