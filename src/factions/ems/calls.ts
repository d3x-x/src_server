...
class EmsCalls {
	...
	cancelCall(id: string) {
		...
		if (medic) mp.blips.delete(medic, 'Чекпоiнт');
	...
	private createCall(player: Player) {
		...
		this.notifyMedics('Надiйшов новий виклик, їдьте швидше!');
	}

	private acceptCall(player: Player, id: string) {
		const call = this.getCall(id);

		if (!call) return mp.events.reject('Цей виклик вже прийнято');
		if (this.hasAcceptedCall(player)) return mp.events.reject('Ви вже прийняли виклик');
		...
		if (victim) {
			...
			mp.blips.create(call.position, { model: 0, color: 1, name: 'Чекпоiнт' }, player);
			hud.showNotification(
				victim,
				'info',
				'Госпiталь прийняв ваш виклик, чекайте на швидку допомогу'
			);
		}
	}
