...
class WantedList {
	...
	private async addItem(
		...
		if (isExists) return mp.events.reject('Цей громадянин уже у розшуку');
	...
	private async arrest(player: Player, userId: string, time: number, reason: string) {
		...
		if (!prison.inZone(player) || !player.entityIsNearby(target?.mp)) {
			return mp.events.reject('Ви маєте бути на територiї в`язницi');
		}
		if (!cuffs.inCuffs(target)) {
			return mp.events.reject('Громадянин має бути у наручниках');
		}
