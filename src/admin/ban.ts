...
class Ban {
	...
	getExpiresDate(data: BanData) {
		return data.permanent ? 'Нiколи' : moment(data.expires).format('DD.MM.YYYY HH:mm');
	}
	...
	private async banPlayer(admin: Player, userId: string, term: string, reason: string) {
		...
		if (!target) return mp.events.reject('Гравець не в мережi');

		if (this.getHoursAmount(term) > 6 && !permissions.hasPermission(admin, 'gm')) {
			return mp.events.reject('Максимальний термiн – 6 годин');
		}
		...
		chat.sendSystem(`${admin.getName()} забанив ${target.getName()} (${reason})`);
	...
	private async unbanPlayer(admin: Player, email: string) {
		if (!permissions.hasPermission(admin, 'gm')) {
			return mp.events.reject('Недостатньо прав!');
		}
		...
		if (!user) return mp.events.reject('Гравець не знайдено');
