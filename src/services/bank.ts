...
class Bank extends Service {
	...
	private async createAccount(player: Player, custom?: string) {
		if (player.bankAccount) return mp.events.reject('Ви вже вiдкрили рахунок');

		if (custom) {
			const isExists = await this.checkExists(custom);

			if (isExists) return mp.events.reject('Цей банкiвський рахунок вже зареєстровано');
	...
	private async cashOut(player: Player, sum: number) {
		if (!player.bankAccount) {
			return mp.events.reject('Для початку оформiть банкiвський рахунок');
		}
	...
	private async transferMoney(player: Player, account: string, value: any) {
		...
		if (!user) return mp.events.reject('Вказаний рахунок не зареєстрований');
