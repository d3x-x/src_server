...
class Login {
	...
	private async signIn(player: Player, email: string, password: string) {
		...
		if (ban.isValid(user)) {
			hud.showNotification(
				player,
				'error',
				`Вас забанили, з причини ${
					user.ban.reason
				}. Термiн блокування: ${ban.getExpiresDate(user.ban)}.`,
				true
			);
	...
	private async checkData(user: UserModel, password: string) {
		if (!user) return { field: 'email', message: 'Облiковий запис не знайдено' };

		const isCorrectPass = await compare(password, user.password);

		if (!isCorrectPass || mp.players.getByDbId(user._id)) {
			return { field: 'password', message: 'Невiрний пароль' };
		}
	}
